        ...
        if !past_header {
            ...
            if t.starts_with("package ") {
                ...
                if lines.peek().is_some_and(|n| {
                    ...
                    if lines.peek().is_some_and(|n| !n.trim_start().starts_with("import ")) {
                        out.push_str(namespaces);
                        if !namespaces.ends_with('\n') {
                            out.push('\n');
                        }
...
pub(crate) fn emit_go_module(
    ...
    for (file_name, code) in modules {
        ...
        // Module bodies are stripped of preambles; re-add package main for go build.
        let file_code = if code.trim_start().starts_with("package ") {
            code.clone()
        } else {
            format!("// Generated by faber package Go assembly\npackage main\n\n{code}")
        };
