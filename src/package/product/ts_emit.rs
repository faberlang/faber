...
struct TsLibraryBindings {
    /// Absolute path to the package-level runtime shim (e.g. `runtime/dom.ts`);
    /// applies to any binding module stem without a per-stem override.
    shim_path: PathBuf,
    /// Per-stem shim overrides, keyed by binding module stem (`canvas2d`).
    /// A package hosting several binding modules wires each stem to its own
    /// runtime shim via `[shims.<stem>]` in the binding manifest.
...
impl TsLibraryBindings {
    /// Runtime shim for one binding module stem: the per-stem override when
    /// declared, otherwise the package-level default shim.
    fn shim_for_stem(&self, stem: &str) -> &Path {
        self.shims
            .get(stem)
            .map(PathBuf::as_path)
            .unwrap_or(self.shim_path.as_path())
    }
...
struct TsBindingFile {
    ...
    /// Per-stem shim overrides (`[shims.<stem>]`), for packages hosting more
    /// than one binding module (web:dom + web:canvas2d, …). Stems without an
    /// entry fall back to the package-level `[shim]`.
    #[serde(default)]
    shims: BTreeMap<String, TsBindingShim>,
}
...
fn load_ts_library_bindings(pkg_root: &Path) -> Result<Option<TsLibraryBindings>, Box<Diagnostic>> {
    ...
    // Package-level default shim; applies to any binding module stem without
    // a per-stem override.
    let default_shim = match &file.shim {
        Some(shim) if !shim.path.trim().is_empty() => {
            Some(resolve_binding_shim(pkg_root, &shim.path, &binding_path)?)
        }
        _ => None,
    };

    // Per-stem shim overrides (`[shims.<stem>]`) so a package hosting several
    // binding modules (web:dom + web:canvas2d, …) wires each stem to its own
    // runtime shim.
    let mut shims: BTreeMap<String, PathBuf> = BTreeMap::new();
    for (stem, shim) in &file.shims {
        if shim.path.trim().is_empty() {
            continue;
        }
        let shim_path = resolve_binding_shim(pkg_root, &shim.path, &binding_path)?;
        shims.insert(stem.clone(), shim_path);
    }

    if default_shim.is_none() && shims.is_empty() {
        return Ok(None);
    }
    ...
    Ok(Some(TsLibraryBindings {
        shim_path: default_shim.unwrap_or_default(),
        shims,
        functions,
    }))
}

/// Resolve a runtime shim path relative to the package root and require it to
/// exist as a file.
fn resolve_binding_shim(
    pkg_root: &Path,
    shim_rel: &str,
    binding_path: &Path,
) -> Result<PathBuf, Box<Diagnostic>> {
    let shim_path = super::super::resolve_package_member(pkg_root, shim_rel, binding_path)?;
...
/// Per-stem genus type-alias tables for binding-module facades.
///
/// Each row maps a binding module stem (the `.fab` leaf stem, `dom`,
/// `canvas2d`, …) to the genus type aliases its generated TS facade must
/// re-export: `(Faber API type name, runtime shim symbol)`. The `dom` row
/// reuses the shared DOM table ([`ts_rewrite::DOM_TYPE_ALIASES`]); further
/// binding modules add one row per stem. A stem without a row simply emits no
/// type-alias block.
const STEM_TYPE_ALIASES: &[(&str, &[(&str, &str)])] = &[
    ("dom", DOM_TYPE_ALIASES),
    ("canvas2d", &[("Canvas2dContext", "WebCanvas2dContext")]),
];

/// Genus type-alias pairs for one binding module stem (`[]` when the stem has
/// no declared type aliases).
fn stem_type_aliases(stem: &str) -> &'static [(&'static str, &'static str)] {
    for (name, aliases) in STEM_TYPE_ALIASES {
        if *name == stem {
            return aliases;
        }
    }
...
///
/// Stems with a per-stem shim override copy their own runtime shim; all other
/// stems share the package-level `[shim]`. Genus type aliases come from the
/// per-stem table ([`stem_type_aliases`]) so every binding module re-exports
/// its genus types through the facade (`Scope`, `Canvas2dContext`, …).
fn emit_ts_binding_facade(
    ...
    let shim_path = bindings.shim_for_stem(stem);
    if shim_path.as_os_str().is_empty() {
        return Err(Box::new(
            product_diag(format!(
                "no TS binding shim for stem `{stem}` (declare `[shim]` or `[shims.{stem}]`)"
            ))
            .with_arg("issue", "product_ts_binding_shim_missing"),
        ));
    }
    ...
    if !runtime_path.is_file() {
        fs::copy(&shim_path, &runtime_path).map_err(|err| io_diag(&runtime_path, err))?;
    }
    ...
    let type_aliases = stem_type_aliases(stem);
    if !type_aliases.is_empty() {
        code.push_str("import type {\n");
        for (_, runtime_name) in type_aliases {
    ...
    if !type_aliases.is_empty() {
        code.push('\n');
        for (api_name, runtime_name) in type_aliases {
...
mod tests {
    ...
    fn canvas2d_facade_uses_own_shim_and_reexports_genus() {
        ...
        let code =
            fs::read_to_string(dir.path().join("faber-web-canvas2d.ts")).expect("read facade");
