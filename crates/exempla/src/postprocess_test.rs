use super::{format_generated_code, lint_generated_code, parse_eslint_output};
use radix::codegen::Target;

#[test]
fn mir_stepper_postprocessing_is_a_no_op() {
    let code = "stepper artifact";

    assert_eq!(
        format_generated_code(Target::MirStepper, code),
        Ok(code.to_owned())
    );
    assert_eq!(
        lint_generated_code(Target::MirStepper, code),
        Ok(code.to_owned())
    );
}
