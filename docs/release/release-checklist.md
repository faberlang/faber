...
| 2 | **Local proof** | L | **builder** → **verifier** | `cargo build --locked --release --bin faber`; `./scripta/release-gate --locked-release-build` (`faber/AGENTS.md:126-133`); optional `../radix/scripta/test --full` for compiler/corpus claims; leakage scan (`process-local-first.md` §4) | gate fail, leak hit, or missing supported matrix leg = stop (candidate incomplete) |
...
| 7 | **Readback** | N (read-only) | **verifier** | `gh release download <tag> --repo faberlang/releases --dir out/readback`; `shasum -c`; verify detached signature | promotion blocked until all required artifacts pass remote readback (`release-contract.md` §4.3) |
