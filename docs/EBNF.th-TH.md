...
# formerly: iteraStmt
# [095] itera_stmt
itera_stmt ::= 'วน' (('ออก' | 'จาก') expression | 'ช่วง' expression) apud_clause? ('คงที่' | 'แปร') (array_pattern | object_pattern | IDENTIFIER) (block_stmt | ergo_joint statement) cape_clause?
# [096] apud_clause
...
The optional `ที่` coordinate clause binds per-axis indices for an `วน ออก`
loop: `วน ออก grid ที่ [r, c] คงที่ cell { … }`. The en reader spelling is
"at" — `วน ออก grid ที่ [r, c]` reads as iterating `grid` at coordinates
`[r, c]`.
