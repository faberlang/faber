...
| **A6. macOS-native packaging decision (OQ8 revisit)** | OQ8 re-reviewed after the archive lifecycle (A1–A5) lands, per `install-channel-matrix.md` conditions: start a `.pkg` only for real OS integration beyond the archive; a drag-a-CLI `.dmg` has no value; native packaging must **not** fork the payload layout or the library model. Decision recorded accepted / explicitly-deferred-with-owner in `decision-ledger.md` + `install-channel-matrix.md`. Any signed channel introduced passes Gatekeeper/notarization **before** being called supported (overlap rule); signing identity leg is `component-release-streamline`'s, not this stage's. | Ledger + matrix rows updated with disposition and owner; no `.pkg`/`.dmg` artifact unless the decision is recorded accepted; no "supported" label on an unsigned channel. | 0.5d |
