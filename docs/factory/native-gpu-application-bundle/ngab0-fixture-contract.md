...
**one scalar host function invoking one prepared submission region containing
one device kernel** (the minimal one-prepared-region case) through the
versioned typed boundary — against a CPU oracle, with the expected evidence
rows named
...
explicit device kernel inside the call's prepared submission region — exactly
the §PackageGraph source-package node — and exactly one cross-boundary call
shape per §Partition/§Abi: one host call → one prepared submission region
containing one kernel.
...
Host side — one native executable
  ...
  composite session surface: dispatch / observe / teardown over ProgramSession
        |
        |  versioned typed boundary — one call, one prepared submission region (§Abi)
        v
Device side
  one prepared submission region containing one device kernel
  scale_kernel(x: f32) -> f32  (typed, target-neutral DeviceProgram)
...
- **One call**: the host function performs exactly one boundary call per
  ...
  side; the prepared submission region — not the kernel — is the call's
  submission unit (§Abi "one call"; NGAB0-R1). The fixture's one-kernel
  region proves the **minimal one-prepared-region** case: one host call → one
  prepared submission region containing one kernel. No host function reaches
  a kernel except through this declared cross-boundary call; anything else
  fails closed in NGAB1.
- **Scalar-first**: the first accepted call shape is scalar-in/scalar-out
  ...
  composition) as multiple kernels per prepared submission region through the
  same ABI without new mechanisms (NGAB1-U4; R3).
...
```faber
// host side: one scalar host function, one boundary call into the prepared region
fn run_scale(x: f32) -> f32 {
    device_call scale_kernel(x)      // one call; the typed boundary carries x
}

// device side: one typed, target-neutral device kernel inside the call's prepared region
...
kernel inside one prepared submission region, scalar in/out through the typed
boundary — not the syntax. NGAB1-U1
...
| --- | --- | --- | --- |
| 1 | **Partition** | The static split in the executed package: one host function (host side = the one native executable) invoking one prepared submission region containing exactly one device kernel through the declared boundary; the region — not the kernel — is the call's submission unit; no second authority on either side; call and entry are the same typed operation | §Partition; §OwnershipMatrix |
...
| 3 | **Manifest identity** | The manifest row for the embedded artifact: `artifact_kind` (admitted variant), `content_sha256` = SHA-256 over the artifact's canonical decoded payload bytes (the NGAB0-R1 re-role of the byte-only `artifact_id`), `packet_sha256` (packet/admission identity over versioned metadata + `content_sha256`), `device_program_version`, `bounds`, `target`; identity verified **before** backend selection | §Manifest (row shape, schema `manifest-2.0.0`, NGAB0-R1); §Verification |
...
scalar host function invoking one prepared submission region containing one
device kernel — the minimal one-region vertical slice) is the proof: one
analyzed package derives validated host MIR → LLVM AND a typed
`DeviceProgram`; the prepared submission region carries the kernel and its
typed device facts (identity, resources, launches, lifetimes, observations) —
typed, not text-parsed; the call links and executes through the existing
llvm-host path (targeted, narrow test)."*
...
- The scalar shape is the extensible first row, not a hard-code: NGAB1-U4
  batches compatible call shapes as multiple kernels per prepared submission
  region through the same ABI without new mechanisms (R3).
