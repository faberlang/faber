...
**Status**: active — lowered 2026-08-10 by planner-4 (goal-check READY); tasking OPEN 2026-08-10. EL-1 landed (faber 7cf2e1f); EL-2 applied + REVERTED + escalated (facade re-export surface load-bearing, verdict escalated_load_bearing_facade, faber f57f28f); EL-2b re-scoped LLVM-only per operator ruling (FMIR/FHIR core, stays ungated); EL-3+ queued behind EL-2b on the shared radix lock.
...
2. **The no-backend build is itself a lane.** Goal Q1 (roundtrip lane mapping): `roundtrip`, `mir` (stepper), and `oracle` use no radix leaf feature — they map to the `--no-default-features` bare exempla build ("frontend + forma + stepper" minimal lane). This also covers the `mir-llvm` gating validation after EL-2b (the `mir-fmir` half is dropped — FMIR/FHIR are core per operator ruling 2026-08-10 and stay ungated). Recorded default; Mind may override at reconcile.
...
4. **Phase-2 gating spans two repos, not one.** Ground truth: `radix-mir-llvm` and `radix-mir-stepper` are unconditional deps of the radix facade; `radix-mir-fmir` is an unconditional dep of **faber** (`faber/Cargo.toml` line 26), with radix's `mir-fmir = []` a no-op. Gating therefore: radix `mir-llvm = ["dep:radix-mir-llvm"]` (and `mir-amd` implies `dep:radix-mir-llvm` + `radix-mir-llvm/mir-amd`); faber `radix-mir-fmir` → `optional = true` + `mir-fmir = ["radix/mir-fmir", "dep:radix-mir-fmir"]` + cfg-gate the faber FMIR consumers. Verified: `radix-mir-stepper/src` does not reference `radix_mir_llvm`, so the goal Q4 concern (llvm unconditional presence load-bearing for the stepper) has no known coupling — EL-2 proved it and reverted+escalated when the facade re-export surface disagreed. **OPERATOR RULING 2026-08-10**: FMIR/FHIR are core functionality — faber `radix-mir-fmir` stays an unconditional dep and faber `mod mir` is NOT gated. LLVM is a regular target output format like wasm/sexp/wgsl/metal and MUST gate cleanly: the escalation sites (cli_descriptor CliAdapter, mir/llvm_text re-export, driver cuda_kernel_descriptor_json) are decoupled in EL-2b, not documented as an exception.
...
EL-1 exempla feature pass-through + cfg gates (rust proof lane)   [Wave A; goal root]
  ├─ EL-2b gate radix-mir-llvm like the sibling formats (FMIR core, stays)  [Wave B; radix; serialized]
...
### EL-2b — Gate `radix-mir-llvm` like the sibling output formats (LLVM = regular lane; FMIR core stays)

**Operator ruling 2026-08-10**: FMIR/FHIR are core functionality — the faber `radix-mir-fmir` gating (EL-2's faber half) is **dropped**; faber `mod mir` stays unconditional. LLVM is a regular target output format like wasm/sexp/wgsl/metal and must gate cleanly behind `mir-llvm`. The EL-2 escalation sites are **decoupled, not documented as an exception** — this is the re-scope of EL-2 (faber `f57f28f` recorded the escalation; hand-6's re-scope proposal, mail `ed350cc4`).

| Field | Content |
| --- | --- |
| **outcome** | A rust-only build compiles no `radix-mir-llvm`: radix `mir-llvm = ["dep:radix-mir-llvm"]` (+ `mir-amd` implies `dep:radix-mir-llvm` + `radix-mir-llvm/mir-amd`); `mod llvm_text` + its `pub use` block get `#[cfg(feature = "mir-llvm")]` — identical treatment to `wasm_text`/`metal_text`/`sexp_text`/`wgsl_text`/`coverage` (the sibling modules are ALL gated; llvm_text is the only ungated one). `full-targets` builds unchanged and green; faber unchanged (FMIR core). |
| **decouple** (the escalation sites) | (a) `crates/radix/src/mir/mod.rs` — `mod llvm_text` + the `pub use llvm_text::{…}` block behind `#[cfg(feature = "mir-llvm")]`; (b) `crates/radix/src/cli_descriptor.rs` — the `CliAdapterField/Plan/RecordFunction` import + `build_cli_adapter_plan`/`build_cli_adapter_fields` cfg-gated or moved to an llvm-gated path (the FCLI encoder stays core; the CliAdapter types already ride the llvm leaf's re-export surface); (c) `crates/radix/src/driver/mod.rs` `cuda_kernel_descriptor_json` + the `--cuda-descriptor` emit path (`tool/commands/emit.rs`) cfg-gated behind the llvm/cuda surface. |
| **write_scope** | radix/crates/radix/Cargo.toml; radix/crates/radix/src/mir/mod.rs; radix/crates/radix/src/cli_descriptor.rs; radix/crates/radix/src/driver/mod.rs; radix/crates/radix/src/tool/commands/emit.rs; the llvm/cuda tests that reference these; faber/docs/factory/per-lane-e2e-validation/ledger.md. NO faber Cargo.toml / `mod mir` changes. |
| **read_scope** | `crates/radix/src/` callers of the three surfaces (llvm_text tests, emit.rs cuda command); faber's llvm-lane feature pass-through (EL-1) to confirm it enables `radix/mir-llvm`. |
| **done_when** | (a) `cargo check -p radix --no-default-features --features hir-rust` exits 0 AND `cargo tree -p radix --no-default-features --features hir-rust` shows no `radix-mir-llvm` compile; (b) `cargo test -p radix --lib` green (shared behavior incl. stepper); (c) default/`full-targets` unchanged — `cargo check -p radix` green + `cargo test -p exempla --lib` from faber green (the faber llvm lane still works via the EL-1 pass-through enabling `radix/mir-llvm`); (d) `git diff --check` clean; (e) one closeout `./scripta/test --stage 1-3`. |
| **validation** | the two feature-limited exact-crate commands above (one run each); `cargo test -p radix --lib`; `cargo test -p exempla --lib` from faber; `./scripta/test` from faber/ (stage 1) at closeout. If a remaining unconditional facade reference surfaces, REVERT and escalate with the offending reference — never force the gate through. |
| **depends_on** | EL-1 (isolation prerequisite). Dispatch-serialized beside EL-3 on the shared radix lock. |
| **non_goals** | No FMIR/FHIR gating (core, operator ruling); no `radix-mir-stepper` gating; no `failable_facts_parts` (EL-3); no `full-targets` membership change; no faber `mod mir` changes. |
| **est_work_tokens** | 10k–20k. **est_basis**: `crate-dep-swap` (ledger 483s / 43 calls — dep + feature wiring, scaled up for the three-site src/** decouple). |
| **dispatch note** | edits radix facade internals → Mind serializes; radix is a hot shared crate. |
