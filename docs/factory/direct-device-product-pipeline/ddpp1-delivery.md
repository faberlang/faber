...
| **write_scope** | `faber/src/package/device/section.rs`, `faber/src/package/device/run.rs`, `faber/src/package/host_factory.rs` (B4–B7); compile-restore hunks in `device/wire.rs`, `device/program.rs`, `device/device_test.rs`, `device/prefill_run_test.rs`, `package/mir/mod.rs` (FmirDeviceSelection `Unknown(_)` arm + `FmirImageError::ArtifactDigestMismatch`), `package/mir/routes.rs`. Hunks only on the landed slice; **no S1/S2 source-identity FNV sites** (`mir/image.rs`, `mir/bin_runner.rs`). |
...
| **write_scope** | `faber/Cargo.toml` (optional deps + `device-runtime`/`host-macos-arm64`/`host-wasm` features + `full-targets` wiring); `src/package/mod.rs` (module gates for `device`, `host_factory`); `src/package/device/mod.rs` (submodule gates incl. `prefill_run`); caller compile-out hunks in `src/cli/mod.rs`, `src/commands/run.rs`, `src/package/manifest.rs`, `src/package/mir/mod.rs` + `mir/routes.rs` (feature-gated route-selection plumbing); tests proving the small build's exclusion. |
...
| **write_scope** | `faber/src/commands/targets.rs` (capability-driven row set + device/host-leaf/device-runtime capability rows keyed to the compiled features); `src/commands/` tests + `src/cli_test.rs` assertions. |
...
| **write_scope** | `faber/src/package/compile.rs` (build-plan result construction), `faber/src/package/artifact_plan.rs` (plan result shape/extensions), `faber/src/package/cmd.rs` (build routing + inspection output), `faber/src/package/manifest.rs` (build-manifest hunks carrying packet identity rows + namespace), new build-plan tests (`compile_test.rs` / `artifact_plan_test.rs`). |
...
| **write_scope** | `faber/src/package/device/section.rs` + `device/wire.rs` (post-U1 packet-consumption hunks — hunk-disjoint from U1's B4–B7 hunks, serialized after U1), new `device/packet_materialization_test.rs`, evidence fixture (`docs/factory/direct-device-product-pipeline/evidence/ddpp1-roundtrip.md` or `.txt` capturing the worked spellings + declared digests). |
...
| **write_scope** | `faber/src/package/device/program.rs` (legality-fact seam hunks on the landed U1 state), `faber/src/package/device/prefill_run.rs` (`build_semantics` seam), legality enforcement tests (`device/program` legality test file — positive CPU `ad` + negative device `ad` in the same file set, never traded). |
